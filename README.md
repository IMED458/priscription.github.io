...
<style>
  ...
  .container { max-width:297mm; margin:auto; background:white; border-radius:8px; overflow:hidden; box-shadow:0 4px 20px rgba(0,0,0,0.1); }

  /* სათაური უკიდურესად პატარა */
  ...
  /* ==================== ბეჭდვა - ზუსტად 2 გვერდი ==================== */
  @media print {
    @page { size: A4 landscape; margin:4mm !important; }
    body, .container { margin:0 !important; padding:0 !important; background:white !important; box-shadow:none !important; }
    ...
    /* საპასპორტე უჯრედები არ ჩანს */
    .info { gap:2px; font-size:7pt !important; }
...
<div class="container">
  <div class="header"><b>პაციენტის დაკვირვების ფურცელი</b><br>თსსუ და ინგოროყვასძ კლინიკა</div>

  <div class="nav">
    <button class="btn active" data-page="1">1</button>
    <button class="btn" data-page="2">2</button>
    <button class="btn" style="background:#059669">შენახვა</button>
    <button class="btn" onclick="print()">ბეჭდვა</button>
  </div>
  ...
  <div class="page active" id="p1">
    ...
    <div class="info">
      <div><label>პაციენტი</label><input type="text" id="fullName" oninput="upd()"></div>
      ...
      <div><label>თარიღი</label><input type="date" id="dateToday"></div>
...
<script>
  const H = [...Array(16).keys()].map(i=>i+9).concat([...Array(9).keys()].map(i=>i+1));
  ...
  function toggle(e){
    if(e.target.classList.contains('check')) e.target.classList.toggle('marked');
  }

  // მედიკამენტები
  let html = `<tr><th class="drug">მედ/დოზა</th>${H.map(h=>`<th class="time">${h}</th>`).join('')}</tr>`;
  for(let i=1;i<=18;i++){
    html += `<tr class="row" data-n="${i}">
      <td class="drug"><input type="text" placeholder="${i}" oninput="check(${i})"></td>
      ${H.map(()=>`<td><div class="check" onclick="toggle(event);check(${i})"></div></td>`).join('')}
    </tr>`;
  }
  document.getElementById('meds').innerHTML = html;

  function check(n){
    const r = document.querySelector(`.row[data-n="${n}"]`);
    const has = r.querySelector('input').value.trim() || r.querySelector('.check.marked');
    r.classList.toggle('filled', !!has);
  }

  // ვიტალები
  const vit = ['პულსი','სისტ.','დიასტ.','MAP','ტ°','სუნთქვა','CVP','FiO2','SaO2'];
  let vhtml = `<tr><th class="drug">პარამეტრი</th>${H.map(h=>`<th class="time">${h}</th>`).join('')}</tr>`;
  vit.forEach(p=> vhtml += `<tr><td class="drug">${p}</td>${H.map(()=>`<td><div class="check" onclick="toggle(event)"></div></td>`).join('')}</tr>`);
  ...
  let ohtml = `<tr><th class="drug">პარამეტრი</th>${H.map(h=>`<th class="time">${h}</th>`).join('')}</tr>`;
  oth.forEach(p=> ohtml += `<tr><td class="drug">${p}</td>${H.map(()=>`<td><input></td>`).join('')}</tr>`);
  ...
  // ბეჭდვა
  window.print = ()=>{
    document.querySelectorAll('.row').forEach(r=>check(r.dataset.n));
    window.print();
  };
</script>
</body>
